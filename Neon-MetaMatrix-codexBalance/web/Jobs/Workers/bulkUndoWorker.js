...
const bulkUndoWorker = new Worker(
  ...
  async (job) => {
    ...
    let undoLease = null;
    const undoWorkerId = operationLeaseService.workerId(WORKER_NAME);
    ...
    try {
      ...
      const writeLock = await storeExecutionLockService.acquireWriteLock({
        ...
        excludeUndoExecutionIdentity: executionId,
      });
      ...
      undoLease = await operationLeaseService.acquireUndoLease({
        shop,
        executionIdentity: executionId,
        workerId: undoWorkerId,
        allowedStates: ["FROZEN", "DISPATCHING"],
      });

      if (!undoLease.acquired) {
        throw new RetryableBulkUndoError(
      ...
      const result = await service.dispatchUndoBulkOperation({
        historyId,
        executionId,
        field: rule.field,
        workerId: undoWorkerId,
      ...
      await operationLeaseService.heartbeatUndoLease({
        shop,
        executionIdentity: executionId,
        workerId: undoWorkerId,
      });
      ...
      await bulkUndoExecutionRepository.markAwaitingShopify({
        ...
        lastSnapshotOrdinal: result.lastSnapshotOrdinal,
        count: result.targetCount,
        jsonlFileBytes: result.jsonlFileBytes,
        dispatchProductCount: result.productCount,
        now: new Date(),
      });
      ...
      await prisma.editHistory.updateMany({
        ...
        data: {
          bulkOperationId: result.bulkOperationId,
          processingBatchId: `${undo.executionIdentity || historyId}:${result.lastSnapshotOrdinal || "start"}`,
          batch: {
            ...batch,
            hasMore: false,
            currentBatchTargetCount: result.targetCount,
          },
          undo: {
            ...undo,
            status: "processing",
      ...
      if (undoLease?.acquired) {
        await operationLeaseService.releaseUndoLease({
          shop,
          executionIdentity: executionId,
          workerId: undoWorkerId,
        }).catch(() => {});
