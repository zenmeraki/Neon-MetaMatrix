...
export async function updateScheduledExportController(req, res) {
  ...
  try {
    ...
    const data = await updateScheduledExport({
      ...
      body: req.body,
      subscription: req.subscription,
      expectedUpdatedAt: req.body?.expectedUpdatedAt || req.headers["if-unmodified-since"] || null,
    });
    ...
    const statusCode =
      error.message === "Session expired"
        ...
        : error.message === "Scheduled export not found"
          ? 404
          : ["SCHEDULED_EXPORT_CONFLICT", "SCHEDULED_EXPORT_DUPLICATE"].includes(error.code)
            ? 409
          : 400;
