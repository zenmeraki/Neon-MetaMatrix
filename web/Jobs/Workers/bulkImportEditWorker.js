...
const MAX_IMPORT_ROWS = Number(process.env.CSV_IMPORT_MAX_ROWS || 10000);
const MAX_IMPORT_PRODUCTS = Number(process.env.CSV_IMPORT_MAX_PRODUCTS || 5000);
  ...
  async (job) => {
    ...
    try {
      ...
      if (
        ...
        !Object.values(columnMappings).includes("id") ||
        Object.values(columnMappings).some((value) => !ALLOWED_IMPORT_FIELDS.has(String(value)))
      ) {
        throw new Error("Invalid import column mappings");
      }
        ...
        stream
          .on("headers", (headers) => {
            if (!Array.isArray(headers) || headers.length === 0 || headers.length > 200) {
              stream.destroy(new Error("CSV headers are invalid"));
            }
          })
          .on("data", (row) => {
            totalRows += 1;
            if (totalRows > MAX_IMPORT_ROWS) {
              stream.destroy(new Error("CSV import row limit exceeded"));
              return;
            }

            buildMappedProductRows(columnMappings, row, productMap);

            if (productMap.size > MAX_IMPORT_PRODUCTS) {
              stream.destroy(new Error("CSV import product limit exceeded"));
            }
          })
          .on("end", resolve)
          .on("error", reject);
