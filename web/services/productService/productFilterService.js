...
function resolveSession(explicitSession, boundSession = null) {
  const session = explicitSession || boundSession;

  if (!session?.shop) {
    throw new Error("Active Shopify session is required");
  }
...
export const productFilterService = {
  ...
  async startBulkOperationToFetchProducts({
    ...
    const safeSession = resolveSession(session);
  ...
  async startBulkOperationToFetchProductVariants({
    session,
    syncLeaseOwner = null,
  }) {
    const safeSession = resolveSession(session);
