...
export const scheduledEditRunRepository = {
  ...
  async markCreatingWithOwner(id, shop, claimedBy, db = prisma) {
    return getClient(db).scheduledEditRun.updateMany({
      where: {
        id,
        shop,
        operationId: null,
        status: "CLAIMED",
        ...(claimedBy ? { claimedBy: String(claimedBy) } : {}),
      },
      data: {
        status: "EXECUTING",
        updatedAt: new Date(),
...
export async function claimDueScheduledEdits(
  limit = 50,
  db = prisma,
  { claimedBy = "scheduled.claim" } = {},
) {
  return db.$queryRaw`
    UPDATE "ScheduledEditRun"
    SET "claimedAt" = now(),
        "claimedBy" = ${claimedBy},
        "executionKey" = COALESCE("executionKey", 'scheduled-edit-run:' || id),
        "status" = 'CLAIMED',
        "updatedAt" = now()
    WHERE id IN (
      SELECT id
