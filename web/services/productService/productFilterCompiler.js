import { compileFilterAst } from "../rules/astSqlCompiler.js";
...
function buildCollectionTypeToken(value) {
...
function jsonValueExpression(alias, column, cast = "text") {
  if (cast === "numeric") {
    return `json_build_object('field', ${alias}.${column}) AS before_value_json`;
  }
  return `json_build_object('field', ${alias}.${column}) AS before_value_json`;
}

function resolveSnapshotFieldConfig(operationField) {
  const key = String(operationField || "").trim();
  const productFieldMap = {
    title: { resource: "PRODUCT", column: "title", cast: "text" },
    status: { resource: "PRODUCT", column: "status", cast: "text" },
    vendor: { resource: "PRODUCT", column: "vendor", cast: "text" },
    productType: { resource: "PRODUCT", column: '"productType"', cast: "text" },
    tags: { resource: "PRODUCT", column: "tags", cast: "text" },
  };
  const variantFieldMap = {
    price: { resource: "VARIANT", column: "price", cast: "numeric" },
    compareAtPrice: { resource: "VARIANT", column: '"compareAtPrice"', cast: "numeric" },
    sku: { resource: "VARIANT", column: "sku", cast: "text" },
    barcode: { resource: "VARIANT", column: "barcode", cast: "text" },
    inventoryQuantity: { resource: "VARIANT", column: '"inventoryQuantity"', cast: "numeric" },
  };

  return productFieldMap[key] || variantFieldMap[key] || null;
}

export function compileProductFilterAstToSql({
  shop,
  mirrorBatchId,
  ast,
  resource = "PRODUCT",
  ...
  const fieldConfig = resolveSnapshotFieldConfig(operationField);
  const effectiveResource =
    String(resource || "").toUpperCase() === "VARIANT" ||
    fieldConfig?.resource === "VARIANT"
      ? "VARIANT"
      : "PRODUCT";

  if (effectiveResource === "VARIANT") {
    const beforeExpr = fieldConfig
      ? jsonValueExpression("v", fieldConfig.column, fieldConfig.cast)
      : "json_build_object('field', NULL) AS before_value_json";
...
ORDER BY product_id ASC, variant_id ASC`,
  ...
  const beforeExpr = fieldConfig
    ? jsonValueExpression("p", fieldConfig.column, fieldConfig.cast)
    : "json_build_object('field', NULL) AS before_value_json";
...
export function buildPrismaNumberFilter(field, operator, value) {
  ...
  switch (op) {
    ...
    case "does not equal":
    case "is not":
      return { NOT: { [field]: { equals: normalizeNumberValue(field, value) } } };
...
export function buildPrismaNotInManualCollectionFilter(operator, value) {
  const op = normalizeOperator(operator);
  const normalizedValue = String(value ?? "").trim().toLowerCase();
  const isTruthy = ["", "true", "1", "yes", "is", "equals"].includes(normalizedValue);
  const manualTypeToken = buildCollectionTypeToken("MANUAL");

  if (!["is", "equals", "="].includes(op)) {
    throw new Error(`Unsupported manual collection operator: ${operator}`);
  }

  if (!isTruthy) {
    throw new Error(`Unsupported manual collection value: ${value}`);
  }
  ...
  return {
    OR: [
      ...
      { collectionsJson: { equals: [] } },
      {
        NOT: {
          collectionsJson: {
            string_contains: manualTypeToken,
          },
        },
      },
    ],
...
export function getProductPrismaWhere(filterParams = [], shop) {
  ...
  for (const [index, rawFilter] of filterParams.entries()) {
    ...
    switch (field) {
      case "search":
        {
          const searchText = normalizeStringValue("search", value);
          if (!searchText) break;
        AND.push({
          OR: [
            { title: { contains: searchText, mode: "insensitive" } },
            { vendor: { contains: searchText, mode: "insensitive" } },
            { productType: { contains: searchText, mode: "insensitive" } },
            { handle: { contains: searchText, mode: "insensitive" } },
            { descriptionText: { contains: searchText, mode: "insensitive" } },
      ...
      case "collection":
        ...
        break;
      case "not_in_manual_collection":
        AND.push(buildPrismaNotInManualCollectionFilter(operator, value));
        break;
