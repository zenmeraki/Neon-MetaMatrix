import { removeStripHtmlTags } from "../../utils/loggerUtils.js";
...
/**
 * Normalizes a tag value for storage and filtering.
 *
 * Rules:
 * - Trim leading/trailing whitespace
 * - Collapse internal whitespace sequences to a single space
 *   ("Summer  Sale" → "Summer Sale", " New  Arrivals " → "New Arrivals")
 * - Preserve original casing (tags are display values; case-insensitive
 *   matching is not applied because Prisma array `has` is exact-match)
 * - Returns null for empty/blank inputs
 *
 * The filter compiler applies the same normalization at query time via
 * buildPrismaArrayStringFilter so stored and searched values always match.
 */
...
export function normalizeNullableDecimal(value, scale = 6) {
  if (value === undefined || value === null || value === "") return null;
  ...
  const trimmedFraction = fractionPart.slice(0, scale).replace(/0+$/, "");
...
export function flattenProduct(product, shop, metaobjectLookup = new Map()) {
  ...
  const tags = Array.isArray(product.tags)
    ? product.tags.map((tag) => normalizeTag(tag)).filter(Boolean)
    : [];
  ...
  return {
    ...
    // descriptionHtml is the only Shopify source field; descriptionText is the
    // deterministic plain-text projection used by search/filter/export reads.
    descriptionHtml: normalizeNullableString(product.descriptionHtml),
    descriptionText: normalizeNullableString(removeStripHtmlTags(product.descriptionHtml)),
    ...
    totalInventoryAuthority: "SHOPIFY_PRODUCT_BULK",
    totalInventoryComputedAt: product.updatedAt ? new Date(product.updatedAt) : new Date(),
...
export function flattenVariant(productId, variant, shop) {
  const price = normalizeNullableDecimal(variant.price);
  const cost = normalizeNullableDecimal(variant.inventoryItem?.unitCost?.amount);
  ...
  if (
    ...
    profitMargin = normalizeNullableDecimal(
  ...
  return {
    ...
    price,
    compareAtPrice: normalizeNullableDecimal(variant.compareAtPrice),
    cost,
