...
import { blastRadiusService } from "../services/execution/blastRadiusService.js";
import { catalogAnomalyService } from "../services/execution/catalogAnomalyService.js";
import { operationReplayService } from "../services/execution/operationReplayService.js";
import {
  ...
  try {
    ...
    if (deterministicInput && deterministicInput.targeting?.ast) {
      ...
      const operation = await merchantOperationRepository.createPlannedOperationForEdit({
        shop: session.shop,
        type: "BULK_EDIT",
        title: "Deterministic bulk edit preview",
        source: "deterministic_preview",
        idempotencyKey: `deterministic-preview:${intentHash}`,
        totalItems: Number(result?.data?.count || result?.data?.targetCount || 0),
        startedAt: null,
      });
      ...
      await merchantOperationRepository.transitionById(operation.id, session.shop, {
        snapshotSetId: snapshot.snapshotSetId,
    ...
    result.data = {
      ...(result.data || {}),
      blastRadius: blastRadiusService.buildEditBlastRadius({
      ...
      anomalies:
        typeof targetSnapshotId === "string" && targetSnapshotId.trim()
          ? await catalogAnomalyService.detectForFrozenTarget({
              shop: session.shop,
              targetSnapshotId: targetSnapshotId.trim(),
...
export const previewReplayOperation = async (req, res) => {
  let session;
  try {
    session = getSessionOrThrow(res);
    const sourceOperationId = String(req.params.id || "").trim();
    if (!sourceOperationId) {
      return res.status(400).json({
        success: false,
        error: "REPLAY_SOURCE_OPERATION_ID_REQUIRED",
      });
    }

    const preview = await operationReplayService.buildReplayPreview({
      shop: session.shop,
      sourceOperationId,
    });
    ...
    await logApiError({
      ...
      err,
      req,
      source: "POST /api/operations/:id/replay/preview",
    });
    return res.status(Number(err?.statusCode) || 400).json({
      success: false,
      error: err?.code || "REPLAY_PREVIEW_FAILED",
      message: err?.message || "Failed to build replay preview",
      details: err?.details || null,
    });
  }
};

export const executeReplayOperation = async (req, res) => {
  let session;
  try {
    session = getSessionOrThrow(res);
    const sourceOperationId = String(req.params.id || "").trim();
    ...
    const replayBody = await operationReplayService.buildReplayExecutionPayload({
      shop: session.shop,
      sourceOperationId,
    ...
    req.body = {
      ...(req.body || {}),
      ...replayBody,
      source: BulkEditSource.REPLAY,
      replayOfOperationId: sourceOperationId,
    };

    return handleBulkEditProduct(req, res);
  } catch (err) {
    await logApiError({
      shop: session?.shop,
      err,
      req,
      source: "POST /api/operations/:id/replay/execute",
    });
    return res.status(Number(err?.statusCode) || 400).json({
      success: false,
      error: err?.code || "REPLAY_EXECUTION_FAILED",
      message: err?.message || "Failed to execute replay",
      details: err?.details || null,
    });
  }
};

export const getOperationSpine = async (req, res) => {
  let session;
  try {
    session = getSessionOrThrow(res);
    ...
    const spine = await loadOperationSpine({
      shop: session.shop,
      operationId,
    });
    ...
    return res.status(200).json({
      ...
      data: {
        ...spine,
        explanation: explainOperationSpine(spine),
      },
    });
  } catch (err) {
    await logApiError({
      ...
      err,
      req,
      source: "GET /api/products/operations/:id/spine",
    });
    return res.status(err?.code === "OPERATION_NOT_FOUND" ? 404 : 400).json({
      success: false,
      error: err?.code || "OPERATION_SPINE_LOAD_FAILED",
      message: err?.message || "Failed to load operation spine",
      details: err?.details || null,
    });
  }
};
...
export const createScheduledEdit = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const frozenExecutionSnapshot = {
        schemaVersion: "scheduled_edit_execution_snapshot.v1",
        scheduledRunId: scheduledRun.id,
        scheduledEditId: history.id,
        shop: session.shop,
        scheduledFor: scheduledAt.toISOString(),
        canonicalFilterSnapshot: Array.isArray(filterParams) ? filterParams : [],
        ...
        sourceTargetSnapshotId: normalizedTargetSnapshotId,
        targetMirrorBatchId: resolvedTarget.mirrorBatchId || null,
        targetSnapshotCount: Number(frozenCount || 0),
        totalItems: Number(count || 0),
        intentId: scheduledIntentId,
        mutationPlanHash: scheduledMutationPlanHash,
        executionPlanVersion: 1,
      };
      const snapshotFingerprint = stableHash(frozenExecutionSnapshot);

      await prisma.scheduledEditExecutionSnapshot.create({
        data: {
          runId: scheduledRun.id,
          shop: session.shop,
          scheduledEditId: history.id,
          canonicalFilterSnapshot: frozenExecutionSnapshot.canonicalFilterSnapshot,
          canonicalRuleSnapshot: frozenExecutionSnapshot.canonicalRuleSnapshot,
          sourceTargetSnapshotId: normalizedTargetSnapshotId,
          snapshotFingerprint,
          executionPlanVersion: 1,
          targetMirrorBatchId: resolvedTarget.mirrorBatchId || null,
