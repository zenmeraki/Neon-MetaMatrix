...
const HistoryTable = memo(function HistoryTable({
  ...
  const [replayHistoryItem, setReplayHistoryItem] = useState(null);
  ...
  const canReplay = useCallback((item) => {
    const status = String(item?.status || "").toLowerCase();
    return status === "completed";
  }, []);

  const closeReplayModal = useCallback(() => {
    if (replayLoading || replayExecuting) return;
    setReplayModalOpen(false);
    setReplayHistoryItem(null);
    setReplayPreview(null);
    setReplayError("");
  }, [replayLoading, replayExecuting]);

  const openReplayModal = useCallback(
    async (history) => {
      if (!canReplay(history)) return;
      const operationId = history?.operationId || history?.id;
      if (!operationId) return;
      setReplayHistoryItem(history);
      setReplayPreview(null);
      setReplayError("");
      ...
      try {
        const response = await fetchWithAuth(
          `/api/products/operations/${operationId}/replay/preview`,
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok || !json?.success) {
          throw new Error(json?.message || json?.error || "Failed to preview replay");
        }
        setReplayPreview(json.data || null);
      } catch (err) {
        setReplayError(err?.message || "Failed to preview replay");
      } finally {
        setReplayLoading(false);
      }
    },
    [canReplay, fetchWithAuth],
  );

  const executeReplay = useCallback(async () => {
    const operationId = replayHistoryItem?.operationId || replayHistoryItem?.id;
    if (!operationId || !replayPreview?.targetSnapshotId) {
      setReplayError("Replay preview is missing a target snapshot.");
      return;
    }
    setReplayExecuting(true);
    setReplayError("");
    try {
      const riskScore = Number(replayPreview?.preflight?.blastRadius?.riskScore || 0);
      const response = await fetchWithAuth(
        `/api/products/operations/${operationId}/replay/execute`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            targetSnapshotId: replayPreview.targetSnapshotId,
            executionFingerprint: replayPreview.executionFingerprint || null,
            riskConfirmation: riskScore >= 80,
          }),
        },
      );
      const json = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(json?.message || json?.error || "Failed to execute replay");
      }
      setReplayModalOpen(false);
      setReplayHistoryItem(null);
      setReplayPreview(null);
      ...
      setReplayError(err?.message || "Failed to execute replay");
    } finally {
      setReplayExecuting(false);
    }
  }, [fetchWithAuth, replayHistoryItem, replayPreview]);
  ...
  const groupedRows = useMemo(() => {
    ...
    (localHistories || []).forEach((item, index) => {
      ...
      const isReplayable = canReplay(item);
      const undoDisabled = !isUndoable || isProcessing || isSyncInProgress;
      const replayDisabled = !isReplayable || isProcessing || isSyncInProgress;
          ...
          <IndexTable.Cell>
            <InlineStack gap="200" wrap={false}>
              ...
              <Button
                ...
                onClick={() => {
                  if (isReplayable) {
                    openReplayModal(item);
                  }
                }}
                disabled={replayDisabled}
              >
                {t("historyReplayButton", { defaultValue: "Replay" })}
              </Button>
            </InlineStack>
  ...
  return (
    <Card padding="0">
      ...
      <Modal
        ...
        title={t("historyReplayModalTitle", { defaultValue: "Replay Operation" })}
        primaryAction={{
          content: t("historyReplayExecuteButton", { defaultValue: "Run replay" }),
          onAction: executeReplay,
          loading: replayExecuting,
          disabled: replayLoading || !replayPreview || replayExecuting,
        }}
        secondaryActions={[
          {
            content: t("cancel", { defaultValue: "Cancel" }),
            onAction: closeReplayModal,
            disabled: replayLoading || replayExecuting,
          },
        ]}
      >
        <Modal.Section>
          <BlockStack gap="300">
            {replayError ? <Banner tone="critical">{replayError}</Banner> : null}
            {replayLoading ? (
              <Text as="p" tone="subdued">
                {t("historyReplayLoading", { defaultValue: "Building replay preview..." })}
              </Text>
            ) : null}
            {!replayLoading && replayPreview ? (
              <BlockStack gap="200">
                <Text as="p" variant="bodyMd">
                  {t("historyReplaySource", {
                    defaultValue: "Source operation: {{id}}",
                    id: replayPreview?.sourceOperationId || "-",
                  })}
                </Text>
                <Text as="p" variant="bodyMd">
                  {t("historyReplayTargetCount", {
                    defaultValue: "Products affected now: {{count}}",
                    count: Number(replayPreview?.drift?.currentCount || 0),
                  })}
                </Text>
                <Text as="p" variant="bodyMd">
                  {t("historyReplayDrift", {
                    defaultValue: "Target drift: +{{added}} / -{{removed}}",
                    added: Number(replayPreview?.drift?.addedCount || 0),
                    removed: Number(replayPreview?.drift?.removedCount || 0),
                  })}
                </Text>
                <Text as="p" variant="bodyMd">
                  {t("historyReplayRisk", {
                    defaultValue: "Risk score: {{score}}/100",
                    score: Number(replayPreview?.preflight?.blastRadius?.riskScore || 0),
                  })}
                </Text>
                <Text as="p" variant="bodyMd">
                  {t("historyReplayAnomalies", {
                    defaultValue: "Catalog anomalies: {{count}}",
                    count: Number(replayPreview?.preflight?.anomalies?.summary?.totalIssues || 0),
                  })}
                </Text>
              </BlockStack>
            ) : null}
