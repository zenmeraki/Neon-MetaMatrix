...
export const csvBulkProductsEdit = asyncHandler(async (req, res) => {
  ...
  const op = await merchantOperationRepository.createPlannedOperationForEdit({
    ...
    idempotencyKey: `import:${req.file.filename}:${Date.now()}`,
    totalItems: 0,
    startedAt: new Date(),
  });
  ...
  const importHistory = await prisma.spreadsheetFile.create({
    data: {
      shop: session.shop,
      operationId: editHistory.operationId,
...
export const queueCsvImportFromS3Controller = asyncHandler(async (req, res) => {
  ...
  const newHistory = await bulkEditHistoryRepository.create({
  ...
  const importDoc = await prisma.spreadsheetFile.create({
    data: {
      shop,
      operationId: newHistory.operationId,
