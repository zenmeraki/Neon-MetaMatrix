...
export async function addScheduledEditJob(name, data = {}, options = {}) {
  ...
  const queueNameForJobId =
    name === "undo-task"
      ? OPERATION_QUEUE_NAMES.UNDO_EXECUTE
      : OPERATION_QUEUE_NAMES.SCHEDULED_DISPATCH;
  const defaultJobId =
    queueNameForJobId === OPERATION_QUEUE_NAMES.SCHEDULED_DISPATCH
      ? data.scheduledRunId
        ? `scheduled:dispatch:${data.shop}:${data.scheduledRunId}`
        : `scheduled:dispatch-history:${data.shop}:${data.historyId}`
      : buildOperationJobId(queueNameForJobId, {
          ...data,
