...
export async function handleProductEditOperation({
  bulkOperationId,
  shop = null,
  ...
  try {
    ...
    if (shop) {
      assertShopMatch({
        jobShop: shop,
        dbShop: history.shop,
        context: "bulk_operation_mutation_history",
        jobId: webhookJobId,
    ...
    const session = await getSession(history.shop);
    assertShopMatch({
      jobShop: history.shop,
      dbShop: session?.shop,
      context: "bulk_operation_mutation_session",
      jobId: webhookJobId,
      entityType: "editHistory",
      entityId: history.id,
    });
    const bulkOperation = await fetchBulkOperationDetails(session, bulkOperationId);
    if (bulkOperation?.id && bulkOperation.id !== bulkOperationId) {
    ...
    if (claimKind === "edit") {
      ...
      return {
        ...
        kind: "edit",
        operationId,
        submissionId: dispatchJobId && Number.isInteger(dispatchAttempt)
          ? `${operationId}:${dispatchJobId}:${dispatchAttempt}`
          : null,
        webhookDeliveryId,
        attempt,
      };
