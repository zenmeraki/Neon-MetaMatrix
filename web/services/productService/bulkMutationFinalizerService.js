...
async function processResultUrl(url, accumulator) {
  if (!url) return;
...
export const bulkMutationFinalizerService = {
  ...
  async collectOutcome({ resultUrl, partialDataUrl }) {
    const accumulator = {
      totalLines: 0,
      successLines: 0,
      failedLines: 0,
    ...
    await processResultUrl(resultUrl, accumulator);
    await processResultUrl(partialDataUrl, accumulator);
