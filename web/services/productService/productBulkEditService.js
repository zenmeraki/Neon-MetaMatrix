...
import {
  FIELD_CONFIGS,
  isVariantLevelField as isVariantLevelBulkField,
...
import { verifyExecutionFingerprint } from "../execution/executionFingerprintService.js";
...
const PREVIEW_BASE_PRODUCT_SELECT = Object.freeze({
  id: true,
  title: true,
  handle: true,
  vendor: true,
...
const PREVIEW_VARIANT_SELECT = Object.freeze({
  id: true,
  productId: true,
  title: true,
  sku: true,
...
const PRODUCT_PREVIEW_SELECTABLE_FIELDS = new Set([
  "title",
  "descriptionHtml",
  "handle",
  "vendor",
...
function buildPreviewProductSelect(field, includeVariants = false) {
  const normalizedField = normalizeField(field);
  const config = FIELD_CONFIGS[normalizedField] || null;
  const select = {
    ...PREVIEW_BASE_PRODUCT_SELECT,
  };
  const fieldName = String(config?.fieldName || normalizedField || "");

  if (
    fieldName &&
    !fieldName.includes(".") &&
    PRODUCT_PREVIEW_SELECTABLE_FIELDS.has(fieldName) &&
    !(fieldName in PREVIEW_BASE_PRODUCT_SELECT)
  ) {
    select[fieldName] = true;
  }

  if (fieldName === "descriptionHtml" || normalizedField === "description") {
  ...
  if (config?.optionPosition || fieldName.includes("option")) {
...
function resolveBulkMutationMode({
  fields = [],
  artifactFormat = "shopify.bulkMutationVariables.productSet.v1",
}) {
  if (artifactFormat === "shopify.bulkMutationVariables.inventoryQuantities.productSet.v1") {
    return PRODUCT_SET_MODE.VARIANT_ONLY;
  }

  return determineMutationMode(fields);
}
...
export default class ProductBulkService {
        ...
        const createdOperation = await operationService.createOperation(
          {
            ...
            startedAt: now,
            replayOfOperationId:
              typeof req?.body?.replayOfOperationId === "string"
                ? req.body.replayOfOperationId.trim() || null
                : null,
          },
          tx,
        );
  ...
  async _bulkOperationEdit(body, subscription) {
    ...
    const preflightFingerprint =
      typeof __preflight?.snapshotFingerprint === "string"
        ...
        : null;
    const preflightBlastRadius =
      __preflight?.blastRadius && typeof __preflight.blastRadius === "object"
        ? __preflight.blastRadius
        : null;
    ...
    await verifyExecutionFingerprint({
      ...
      expectedExecutionFingerprint:
        typeof normalizedBody.executionFingerprint === "string"
          ? normalizedBody.executionFingerprint.trim()
          : "",
      mirrorBatchId: resolvedTarget.mirrorBatchId || null,
      canonicalFilterAstHash:
        resolvedTarget?.canonicalQueryHash ||
      ...
      actionPayload: {
        ...
        searchKey: resolvedSearchKey,
        replaceText: resolvedReplaceText,
        supportValue: normalizedBody.supportValue,
        locationId: normalizedBody.locationId || null,
        mutationPlanHash,
      },
      fieldVersionPayload: {
  ...
  async _bulkOperationHelper({
    ...
    try {
      ...
      const mode = resolveBulkMutationMode({
      ...
      if (
        artifactFormat === "shopify.bulkMutationVariables.inventoryQuantities.productSet.v1"
      ) {
        for (const line of jsonlLines) {
          const parsed = JSON.parse(line);
          const variants = parsed?.productSet?.variants;
          const hasInventoryQuantities = Array.isArray(variants)
            ? variants.some(
                (variant) =>
                  Array.isArray(variant?.inventoryQuantities) &&
                  variant.inventoryQuantities.length > 0,
              )
            : false;

          if (!hasInventoryQuantities) {
            const error = new Error("INVENTORY_LOCATION_PAYLOAD_REQUIRED");
            error.code = "INVENTORY_LOCATION_PAYLOAD_REQUIRED";
            throw error;
          }
        }
      }
...
async _preparingBulkOperation({ historyId }) {
  ...
  async trackEditProducts({
    ...
    try {
      ...
      const select = buildPreviewProductSelect(field, isVariant);
      if (isVariant && select.variants) {
        select.variants = {
          ...select.variants,
          where: {
            shop: this.session.shop,
