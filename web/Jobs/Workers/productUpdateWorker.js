    ...
    try {
      ...
      if (!activeCatalogBatchId) {
        await recordMirrorAnomaly({
          shop,
          severity: "high",
          type: "webhook_without_active_catalog_batch",
          entityType: "product",
          entityId: id,
          message: "Product update webhook skipped because the shop has no active catalog batch",
          details: { worker: "productUpdateWorker" },
        }).catch(() => {});

        return { skipped: true, reason: "missing_active_catalog_batch" };
      }
