...
async function finalizeScheduledRunStrict({ exportJobId, status, errorMessage = null }) {
  const maxAttempts = 3;
  let lastError = null;
  const lockKey = `finalize-export:${exportJobId}`;
  const finalizeLock = await tryAdvisoryLock(prisma, lockKey, false);
  if (!finalizeLock) {
    logger.info("Skipped scheduled export run finalization because lock is held", {
      worker: WORKER_NAME,
      queue: QUEUE_NAME,
      exportJobId,
      status,
