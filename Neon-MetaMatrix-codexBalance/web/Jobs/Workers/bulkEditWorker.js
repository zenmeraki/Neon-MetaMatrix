...
async function claimHistoryExecution(historyId, shop, executionId, jobId, attempt) {
  ...
  if (history.executionState === BULK_EDIT_EXECUTION_STATES.PLANNED) {
    return { state: "not_queued", history };
  }
  ...
  assertValidTransition({
    ...
    to: BULK_EDIT_EXECUTION_STATES.DISPATCHING,
  });
...
async function processBulkEdit(job) {
  ...
  let executionLeaseIdentity = null;
  ...
  try {
    ...
    if (["terminal", "awaiting_shopify", "finalizing", "already_running", "not_claimed", "not_queued", "stale_dispatch_failed"].includes(claimResult.state)) {
    ...
    executionLeaseIdentity = history.executionIdentity;

    executionLease = await operationLeaseService.acquireEditHistoryLease({
      shop,
      historyId,
      executionIdentity: executionLeaseIdentity,
      workerId: operationLeaseService.workerId(WORKER_NAME),
      allowedStates: [BULK_EDIT_EXECUTION_STATES.DISPATCHING],
    });
    ...
    await operationLeaseService.heartbeatEditHistoryLease({
      shop,
      historyId,
      executionIdentity: executionLeaseIdentity,
      workerId: executionLease.workerId,
    });

    if (!formattedProducts && !jsonlFilePath) {
      assertValidTransition({
        from: BULK_EDIT_EXECUTION_STATES.DISPATCHING,
        to: BULK_EDIT_EXECUTION_STATES.COMPLETED,
      });
    ...
    assertValidTransition({
      from: BULK_EDIT_EXECUTION_STATES.DISPATCHING,
      to: BULK_EDIT_EXECUTION_STATES.AWAITING_SHOPIFY,
    });
    ...
    if (executionLease?.acquired && executionLeaseIdentity) {
      await operationLeaseService.releaseEditHistoryLease({
        shop,
        historyId,
        executionIdentity: executionLeaseIdentity,
        workerId: executionLease.workerId,
