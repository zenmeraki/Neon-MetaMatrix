...
function getUserFacingMessage(error) {
  if (error.message === "Session expired") return "Session expired";
  if (error.message === "Product code snippet not found") {
    return "Product code snippet not found";
  }

  if (
    error.message === "Unsupported snippet status" ||
    error.message === "Archived snippets cannot be updated" ||
    error.message === "Use delete to archive a snippet" ||
    error.message === "New snippets cannot be created as archived" ||
    error.message === "Archived snippets cannot be previewed" ||
    error.message === "A product selection is required to preview a snippet"
  ) {
    return error.message;
  }

  if (error.message?.includes("validation")) {
    return "Snippet validation failed";
  }
...
export async function createProductCodeSnippetController(req, res) {
  ...
  try {
    ...
    return res.status(getStatusCode(error)).json(errorResponse(getUserFacingMessage(error)));
  }
}
