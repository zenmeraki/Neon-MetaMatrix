...
async function claimImportHistoryForShop(historyId, shop) {
  ...
  if (current.executionState === BULK_EDIT_EXECUTION_STATES.PLANNED) {
    assertValidTransition({
      from: BULK_EDIT_EXECUTION_STATES.PLANNED,
  ...
  if (current.executionState === BULK_EDIT_EXECUTION_STATES.FAILED) {
    assertValidTransition({
      from: BULK_EDIT_EXECUTION_STATES.FAILED,
      to: BULK_EDIT_EXECUTION_STATES.QUEUED,
    });
  ...
  assertValidTransition({
    from: BULK_EDIT_EXECUTION_STATES.QUEUED,
...
const bulkImportEditWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      executionLeaseIdentity = history.executionIdentity ?? null;
