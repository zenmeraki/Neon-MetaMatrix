...
const PRODUCT_QUERY_DB_TIMEOUT_MS = Math.max(
  Number(process.env.PRODUCT_QUERY_DB_TIMEOUT_MS || 15000),
  1000,
);
...
export async function getProductsWithFilters({
  ...
  const { totalCount, hasNextPage, products, nextCursor } =
    await runWithDbStatementTimeout(PRODUCT_QUERY_DB_TIMEOUT_MS, async (tx) => {
      const [totalCountValue, idRows] = await Promise.all([
      ...
      const hasNextPageValue = idRows.length > normalizedLimit;
      const pageRows = hasNextPageValue ? idRows.slice(0, normalizedLimit) : idRows;
      const productIds = pageRows.map((row) => row.id);
      const nextCursorValue =
        hasNextPageValue && productIds.length
          ? productIds[productIds.length - 1]
          : null;

      const productsValue = await productMirrorRepository.findProductSummariesForFrozenTarget(
        {
          shop,
          mirrorBatchId,
          productIds,
...
async function resolveAdvancedFilterConstraints({
  ...
  for (const clause of clauses) {
    const rows = await runWithDbStatementTimeout(
      PRODUCT_QUERY_DB_TIMEOUT_MS,
      (tx) => tx.$queryRaw(clause),
      dbClient,
    );
...
async function runWithDbStatementTimeout(timeoutMs, operation, client = prisma) {
