  ...
  while (hasMore) {
    ...
    for (const product of products) {
      ...
      matchedStateUpdates.push({
        ...
        mirrorBatchId: product.mirrorBatchId,
        lastMatchedAt: now,
      });
...
export async function persistMatchedStateUpdates(rule, matchedStateUpdates = [], run = null) {
  await automaticProductRuleStateRepository.bulkUpsertStates({
    automaticProductRuleId: rule.id,
    shop: rule.shop,
    runId: run?.id,
    executionKey: run?.executionKey,
    rows: matchedStateUpdates.map((update) => ({
      productId: update.productId,
      data: {
        mirrorBatchId: update.mirrorBatchId,
...
export async function persistAppliedStateUpdates(rule, appliedStateUpdates = [], run = null) {
  await automaticProductRuleStateRepository.bulkUpsertStates({
    ...
    runId: run?.id,
    executionKey: run?.executionKey,
    rows: appliedStateUpdates.map((update) => ({
      productId: update.productId,
      data: {
        mirrorBatchId: update.mirrorBatchId,
        lastMatchedAt: update.lastMatchedAt,
