...
import {
  transitionMerchantOperation,
  projectOperationToEditHistory,
} from "../services/merchantOperationStateService.js";
...
export const bulkEditHistoryRepository = {
  async applyProjectionUpdate({ where, data }, db = prisma) {
    return getClient(db).editHistory.updateMany({ where, data });
  },
  async create(data, db = prisma) {
    const operation =
      data.operationId
        ...
        : await merchantOperationRepository.createPlannedOperationForEdit(
            {
              shop: data.shop,
              type: data.type === "Scheduled edit" ? "SCHEDULED_EDIT" : "BULK_EDIT",
              title: typeof data.type === "string" ? data.type : "Bulk edit",
              source: "write_through",
              idempotencyKey: `edit-history-create:${data.executionIdentity || data.startedAt?.toISOString() || Date.now()}`,
    ...
    const created = await getClient(db).editHistory.create({
      data: buildCreateData({
        ...data,
        operationId: operation?.id || data.operationId || null,
      }),
    });
    await merchantOperationRepository.createForEditHistory(created, db);
    return created;
  },
  ...
  async movePlannedToQueued(
    ...
    const updated = await getClient(db).editHistory.updateMany({
      where: {
        id,
        shop,
        executionState: BULK_EDIT_EXECUTION_STATES.PLANNED,
    ...
    const linked = await getClient(db).editHistory.findFirst({
      where: { id, shop },
      select: { operationId: true },
    });
    if (updated.count && linked?.operationId) {
      ...
      await projectOperationToEditHistory({
        shop,
        editHistoryId: id,
        operationId: linked.operationId,
      }, db);
