...
export default function RecurringHistory() {
  ...
  const fetchRecurringEdits = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await fetchWithAuth("/api/recurring-edits");
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      const response = await fetchWithAuth(`/api/recurring-edits/${id}`, {
        method: "PATCH",
  ...
  const handleCancelEdit = async (id) => {
    try {
      const response = await fetchWithAuth(`/api/recurring-edits/${id}`, { method: "DELETE" });
      const data = await response.json();
