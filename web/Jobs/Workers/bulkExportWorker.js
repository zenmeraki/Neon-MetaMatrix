...
import { getFrozenTargetItems } from "../../services/productService/productTargetingService.js";
import {
...
const PRODUCT_FIELD_RESOLVERS = {
  ...
  collections: (p) => {
    if (!Array.isArray(p.collectionTitles)) return "";
    return p.collectionTitles.filter(Boolean).join(", ");
  },
...
const resolveTargetCatalogBatchId = (exportJob) =>
  exportJob?.targetCatalogBatchId || exportJob?.targetMirrorBatchId || null;

const getTargetBatchWhere = (targetCatalogBatchId) =>
  targetCatalogBatchId ? { catalogBatchId: targetCatalogBatchId } : {};
...
async function getCollectionTitlesByProductId({ shop, catalogBatchId, productIds }) {
  if (!catalogBatchId || !Array.isArray(productIds) || productIds.length === 0) {
    return new Map();
  }

  const rows = await prisma.productCollectionMembership.findMany({
    where: {
      shop,
      catalogBatchId,
      productId: {
    ...
    distinct: ["productId", "collectionTitle"],
    orderBy: [{ productId: "asc" }, { collectionTitle: "asc" }],
  });

  const collectionTitlesByProductId = new Map();
  for (const row of rows) {
    if (!row.collectionTitle) continue;

    const current = collectionTitlesByProductId.get(row.productId) || [];
    current.push(row.collectionTitle);
    collectionTitlesByProductId.set(row.productId, current);
  }

  return collectionTitlesByProductId;
}
    ...
    try {
      ...
      leaseRenewal = startExclusiveShopWorkRenewal(lock, {
        onRenewalError: (error) => {
          logger.error("Failed to renew bulk export shop lease", {
            shop,
            exportJobId,
            message: error.message,
          });
      ...
      const pageSize = 500;
      let targetCursorKey = null;
      let hasMore = true;
      let totalRows = 0;
      const targetCatalogBatchId = resolveTargetCatalogBatchId(exportJob);

      while (hasMore) {
        assertExclusiveShopWorkLeaseActive(shopLease);
        const snapshotPage = await getFrozenTargetItems({
          ownerType: "EXPORT_JOB",
          ownerId: exportJobId,
          shop,
          targetSnapshotSetId: exportJob.targetSnapshotSetId,
          limit: pageSize,
          targetCursorKey,
        });

        const productIds = [...new Set(snapshotPage.rows.map((row) => row.productId))];
        ...
        const products = await prisma.product.findMany({
          where: {
            shop,
            id: { in: productIds },
            ...getTargetBatchWhere(targetCatalogBatchId),
          },
          include: {
            variants: {
              orderBy: { id: "asc" },
        ...
        const collectionTitlesByProductId = await getCollectionTitlesByProductId({
          shop,
        ...
        for (const productId of productIds) {
          ...
          product.collectionTitles =
            collectionTitlesByProductId.get(productId) || [];
        ...
        targetCursorKey = snapshotPage.targetCursorKey;
    ...
    stack: err.stack,
      ...
      logBatchEvent("catalog_batch_export", {
        ...
        oldMirrorBatchId:
          ...
          exportJob.targetMirrorBatchId !== targetCatalogBatchId
            ? exportJob.targetMirrorBatchId
            : null,
