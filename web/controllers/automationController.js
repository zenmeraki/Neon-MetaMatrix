...
import {
  ...
  saveShadowApproval,
} from "../services/automation/shadowApprovalService.js";
...
export const promoteAutomationRunController = async (req, res) => {
  try {
    ...
    const fingerprints = await buildRunFingerprints({
      shop: session.shop,
      mirrorBatchId: run.mirrorBatchId,
      executionPlanIds: run.executionPlanIdsJson,
      snapshotSetIds: run.snapshotSetIdsJson,
    });
    await saveShadowApproval({
      shop: session.shop,
      automationRuleId: id,
      approval: {
        approvedAt: new Date().toISOString(),
        approvedBy: session?.id || session?.shop || "unknown",
        sourceRunId: run.id,
        executionFingerprint: fingerprints.executionFingerprint,
        snapshotFingerprint: fingerprints.snapshotFingerprint,
        mirrorBatchId: fingerprints.mirrorBatchId,
      },
    });
    ...
    return res.status(error?.statusCode || 400).json({
      success: false,
      error: error?.code || "AUTOMATION_PROMOTION_FAILED",
...
export const previewAutomationRunController = async (req, res) => {
  try {
    ...
    return res.status(error?.statusCode || 400).json({
      success: false,
      error: error?.code || "AUTOMATION_PREVIEW_FAILED",
