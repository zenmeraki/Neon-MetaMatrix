...
This repository is a real npm workspace. The root `package.json` owns workspace-level commands and dependency resolution for:

- `web`: the Express/Shopify backend.
- `web/frontend`: the Vite/React frontend.
- `extensions/*`: Shopify extensions when present.

Backend code is organized by shared infrastructure and domain modules:

- `web/config`, `web/middleware`, `web/routes`, `web/controllers`, `web/services`, and `web/repositories` hold the existing backend layers.
- `web/jobs` contains queues, workers, cron jobs, and job bootstrap code.
- `web/modules/<domain>` contains business-specific helpers that should not live in generic `web/utils`.
- `web/utils` is reserved for cross-cutting infrastructure helpers such as logging, errors, cache wrappers, sessions, metrics, and worker telemetry.
- `web/prisma` contains the active Prisma schema and migrations.

Prefer adding new or heavily modified feature code under `web/modules/<domain>` when it is domain-owned. Keep generic helpers in `web/utils` only when they are genuinely reusable across domains.
