...
import { productTargetRepository } from "../filterPlanner/productTargetRepository.js";
import { targetSnapshotService } from "../filterPlanner/targetSnapshotService.js";
...
export async function resolveCanonicalProductTarget({
  ...
  if (!explicitWhere && safeProductIds.length === 0) {
    const pinnedMirrorBatchId =
      typeof requestedMirrorBatchId === "string"
        ? requestedMirrorBatchId.trim()
        : "";
    const mirrorBatchId =
      pinnedMirrorBatchId || (await getActiveMirrorBatchId(safeShop));
    ...
    const [countResult, pageResult] = await Promise.all([
      productTargetRepository.count({
        filterParams,
        shop: safeShop,
        mirrorBatchId,
        useCache: !freeze,
      }),
      productTargetRepository.page({
        filterParams,
        shop: safeShop,
        mirrorBatchId,
        page,
        limit: Math.min(limit, sampleLimit),
        cursorId,
    ...
    if (freeze && ownerType && ownerId) {
      frozenCount = await targetSnapshotService.freeze({
        ownerType,
        ownerId,
        shop: safeShop,
        mirrorBatchId,
    ...
    return {
      mirrorBatchId,
      where: null,
      canonicalFilterKey: countResult.canonicalFilterKey,
      count: countResult.count,
      sampleProducts: pageResult.rows,
      frozenCount,
      pagination: {
        ...
        hasPrevPage: page > 1,
        endCursor: pageResult.pagination.endCursor,
      },
    };
  }
