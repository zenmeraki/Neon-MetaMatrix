...
const RecurringHistoryTable = memo(
 ...
 <Badge tone="info">
    ...
    const handleToggleStatus = async (id, currentStatus) => {
      ...
      try {
        const response = await fetchWithAuth(`/api/products/update-recurring-edit/${id}/toggle`, {
          method: "PUT",
    ...
    }, [deleteRecurringItem, fetchWithAuth]);
    ...
    const rows = localHistories.map((item, index) => {
      const {
        _id,
        title,
        status,
      ...
      return (
        <IndexTable.Row id={String(_id)} key={String(_id)} position={index}>
          <IndexTable.Cell>
          ...
          <IndexTable.Cell>{renderFrequencyBadge(frequency)}</IndexTable.Cell>
          <IndexTable.Cell>
