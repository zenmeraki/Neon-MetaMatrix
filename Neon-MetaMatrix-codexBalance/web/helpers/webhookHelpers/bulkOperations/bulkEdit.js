...
async function finalizeUndoSuccess(history) {
  ...
  if (hasMore) {
    await bulkUndoExecutionRepository.markBatchSucceeded({
      shop: history.shop,
      executionIdentity: undo.executionIdentity || history.executionIdentity || history.id,
      hasMore: true,
      now: new Date(),
    });
  ...
  const completedAt = new Date();
  await bulkUndoExecutionRepository.markCompleted({
    shop: history.shop,
    executionIdentity: undo.executionIdentity || history.executionIdentity || history.id,
    now: completedAt,
  });
