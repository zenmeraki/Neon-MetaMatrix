...
class UndoEditService {
  ...
  async undoEdit({ id: historyId, shop, idempotencyKey = null }) {
    if (!historyId || !shop) {
  ...
  async prepareUndoBatch({ historyId, executionId, limit = 75 }) {
    ...
    for (const snapshotRow of snapshotRows) {
      const ids = Array.isArray(snapshotRow.changeRecordIds)
        ...
        : [];
      if (!ids.length) {
        throw codedError("UNDO_SNAPSHOT_CHANGE_IDS_REQUIRED");
      }
    ...
    const hashRowsByProductId = new Map();
    ...
    for (const change of changes) {
      ...
      const existingHashes = hashRowsByProductId.get(change.productId) || [];
      existingHashes.push(hashUndoChangeRow(change));
      hashRowsByProductId.set(change.productId, existingHashes);
    }
    ...
    for (const snapshotRow of snapshotRows) {
      ...
      const actualHash = hashUndoChangeSet(
        hashRowsByProductId.get(snapshotRow.productId) || [],
      );
