...
async function getHydratedRule(id, shop) {
  ...
  const [statusCounts, latestRuns] = await Promise.all([
    automaticProductRuleRunRepository.groupStatusCounts([rule.id], shop),
    automaticProductRuleRunRepository.findLatestRuns([rule.id], shop),
  ]);
...
export async function listAutomaticProductRules({ shop }) {
  ...
  const [statusCounts, latestRuns] = await Promise.all([
    automaticProductRuleRunRepository.groupStatusCounts(ids, shop),
    automaticProductRuleRunRepository.findLatestRuns(ids, shop),
  ]);
...
export async function updateAutomaticProductRule({
  ...
  await automaticProductRuleRepository.updateById(existing.id, shop, {
