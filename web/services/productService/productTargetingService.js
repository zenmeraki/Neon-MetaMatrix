...
export async function computeFrozenTargetIdsHash({
  ownerType,
  ownerId,
  shop,
  pageSize = 1000,
  ...
  while (true) {
    const page = await getFrozenTargetProductIds({
      ...
      ownerId,
      shop: safeShop,
      limit: normalizePositiveInt(pageSize, 1000, 5000),
      cursorOrdinal,
    });
    ...
    for (const row of rows) {
      ...
      if (!firstProductId) firstProductId = productId || null;
      lastProductId = productId || null;
      hash.update(`${ordinal}|${productId}|${variantId}\n`);
      total += 1;
    }
  ...
  hash.update(`total:${total}|first:${firstProductId || ""}|last:${lastProductId || ""}`);
