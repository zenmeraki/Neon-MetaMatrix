...
import { getStoreLifecycleState } from "../../utils/sessionHandler.js";
import { sanitizeForSpreadsheetCell } from "../../utils/spreadsheetSecurity.js";
import {
...
function buildTenantScopedTempFilePath(shop, exportJobId, filename) {
  ...
  return path.join(os.tmpdir(), `${safeShop}-${exportJobId}-${safeFilename}`);
}
...
const bulkExportWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      if (!store || store.isUnInstalled || !store.accessToken) {
        await prisma.exportJob.updateMany({
          ...
          data: {
            status: "CANCELLED",
            executionState: EXPORT_EXECUTION_STATES.FAILED,
            failureStage: "shop_inactive",
            completedAt: new Date(),
            error: buildExportExecutionError({
              code: "shop_inactive",
              stage: "export_worker",
              message: "Shop is no longer active for export execution",
              retryable: false,
              details: {
                worker: WORKER_NAME,
                queue: QUEUE_NAME,
      ...
      filePath = buildTenantScopedTempFilePath(shop, exportJob.id, exportJob.filename);
        ...
        for (const productId of productIds) {
          ...
          if (!variants.length) {
            ...
            for (const field of fields || exportJob.fields) {
              ...
              if (productResolver) {
                row[field] = sanitizeForSpreadsheetCell(productResolver(product));
              }
            }
          ...
          for (let index = 0; index < variants.length; index += 1) {
            ...
            for (const field of fields || exportJob.fields) {
              const productResolver = PRODUCT_FIELD_RESOLVERS[field];
              if (productResolver) {
                row[field] = sanitizeForSpreadsheetCell(
                  index === 0 ? productResolver(product) : "",
                );
              }

              const variantResolver = VARIANT_FIELD_RESOLVERS[field];
              if (variantResolver) {
                row[field] = sanitizeForSpreadsheetCell(variantResolver(variant));
              }
            }
