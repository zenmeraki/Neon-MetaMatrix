...
import { assertShopIsActive, getSession } from "../utils/sessionHandler.js";
...
export async function executeRecurringEditRun(runId, shopFromJob = null) {
  ...
  try {
    await assertShopIsActive(recurringEdit.shop);
  } catch (_error) {
    await markRunSkipped(run, recurringEdit, "Shop is no longer active");
