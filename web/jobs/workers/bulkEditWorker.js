...
function normalizePlannedValue(value) {
...
function buildPreparedChangesFromPlan(planJson) {
  const grouped = new Map();
  for (const mutation of Array.isArray(planJson?.mutations) ? planJson.mutations : []) {
    if (!mutation?.productId || mutation?.variantId) continue;
    const current = grouped.get(mutation.productId) || {
      productId: mutation.productId,
      title: "",
      productFieldChanges: [],
      variantFieldChanges: [],
    };
    current.productFieldChanges.push({
      field: mutation.field,
      oldValue: normalizePlannedValue(mutation.beforeValueJson),
      newValue: normalizePlannedValue(mutation.afterValueJson),
    });
    grouped.set(mutation.productId, current);
  }
  return Array.from(grouped.values());
}

async function loadPreparedExecutionPayload({ executionPlan, shop, operationId }) {
  const artifact = executionPlan?.planJson?.preparedMutationArtifact || null;
  const verified = await assertPreparedMutationArtifactReady({ artifact });
  const content = await fs.readFile(verified.path, "utf8");
  const formattedProducts = content
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
  const changes = buildPreparedChangesFromPlan(executionPlan.planJson);
  const lastChange = changes[changes.length - 1] || null;

  await operationEventRepository.emit({
    shop,
    operationId,
    type: "PREPARED_MUTATION_ARTIFACT_LOADED",
    payload: {
  ...
  return {
    ...
    changes,
    hasMore: false,
    lastProductId: lastChange?.productId || null,
    lastOrdinal: Number(executionPlan?.planJson?.mutationCount || formattedProducts.length),
    batchId: artifact.artifactId,
...
async function processBulkEdit(job) {
  ...
  try {
    ...
    const claimedExecutionPlan = await claimExecutionPlanForDispatch({ executionPlanId, shop });
    ...
    const preparedExecutionPayload = await loadPreparedExecutionPayload({
      executionPlan: claimedExecutionPlan.plan,
      shop,
      operationId: activeOperationId,
    });
    ...
    if (leaseLost) {
      throw new RetryableBulkEditError(
        "Bulk edit execution lease was lost",
    ...
    if (!shopifyBulkOperationId) {
      ...
      if (submissionClaim.count !== 1) {
        ...
        const result = await service._bulkOperationHelper({
          ...
          artifactFormat:
            executionPlan?.planJson?.preparedMutationArtifact?.format ||
            "shopify.bulkMutationVariables.productSet.v1",
        });
    ...
    await operationService
      .checkpointExecution({
        ...
        workerId: WORKER_ID,
        partitionOrdinal: 1,
        targetOrdinal: Number(lastOrdinal || 0),
        heartbeatAt: new Date(),
      })
      .catch(() => {});
