...
export async function formatAndSyncProductsToDB({
  ...
  try {
    ...
    const MAX_PENDING_CHILDREN = Number(
      process.env.PRODUCT_SYNC_MAX_PENDING_CHILDREN || 50_000,
    );
    ...
    const flushProductsAndVariants = async () => {
      ...
      const insertMetrics = await insertProductMirrorBatch({
        productRows,
        variantRows,
        syncBatchId,
      });

      totalProductsProcessed += insertMetrics.productInsertCount;
      totalVariantsProcessed += insertMetrics.variantInsertCount;

      console.log(`[sync:flush] shop=${shop} totalProductsProcessed=${totalProductsProcessed} totalVariantsProcessed=${totalVariantsProcessed} batchInsertTimeMs=${insertMetrics.batchInsertTimeMs}`);
    ...
    for await (const line of rl) {
      ...
      if (pendingChildren.size > MAX_PENDING_CHILDREN) {
        throw new Error(
          `Product sync pending child buffer exceeded ${MAX_PENDING_CHILDREN}; use domain-split sync for this shop`,
        );
      }
    }
