...
import { bulkUndoFinalizerService } from "../../services/productService/bulkUndoFinalizerService.js";
...
const bulkOperationMutationWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      const undoFinalized =
        await bulkUndoFinalizerService.tryFinalizeUndoBulkOperation({
          shop,
          bulkOperationId,
        });

      if (undoFinalized?.handled) {
        return {
          success: true,
          routedTo: "bulk_undo_finalizer",
          shop,
          bulkOperationId,
          result: undoFinalized,
        };
      }
      ...
      return {
        success: true,
        routedTo: "product_edit_finalizer",
        shop,
        bulkOperationId,
        result,
      };
