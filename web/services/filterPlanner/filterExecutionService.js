...
const BITMAP_CACHE_OPERATIONS = new Set(["preview", "export", "facet", "count"]);
...
function isBitmapCacheEligible(operation) {
  return BITMAP_CACHE_OPERATIONS.has(String(operation || "").trim().toLowerCase());
}
...
async function executeClickHousePlanAllIds(plan, shop, mirrorBatchId, totalCount) {
...
async function seedBitmapCache({
  ...
  if (!isBitmapCacheEligible(operation)) {
    return;
  }
  ...
  try {
    const allProductIds =
      plan.engine === "clickhouse"
        ? await executeClickHousePlanAllIds(plan, shop, mirrorBatchId, totalCount)
