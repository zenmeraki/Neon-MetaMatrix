...
function normalizeSort(sort = {}) {
  ...
  return {
    column,
    direction,
    isProductId: column === "product_id",
  };
}

function buildProductIdSubquery({ where, sort }) {
  const normalizedSort = normalizeSort(sort);

  if (normalizedSort.isProductId) {
    return {
      sql: `
        ...
        WHERE ${where}
      `.trim(),
      orderBy: `product_id ${normalizedSort.direction}`,
    };
  }
  ...
  return {
    sql: `
      SELECT
        product_id,
        any(${normalizedSort.column}) AS sort_value
      FROM product_variant_flat
      WHERE ${where}
      GROUP BY product_id
    `.trim(),
    orderBy: `sort_value ${normalizedSort.direction}, product_id ASC`,
  };
}
...
function compileStringPredicate(column, operator, value, context) {
  if (operator === "in") {
    ...
    return `lowerUTF8(${column}) IN (${placeholders
      .map((item) => `lowerUTF8(${item})`)
      .join(", ")})`;
  }
...
function compileDatePredicate(column, operator, value, context) {
  if (operator === "in") {
    ...
    return `toDate(${column}) IN (${placeholders
      .map((item) => `toDate(parseDateTimeBestEffort(${item}))`)
      .join(", ")})`;
  }
...
function compilePredicateToClickHouse(node, context) {
  ...
  if (!column) {
    throw new Error(
      `ClickHouse column mapping missing for field ${node.field}`
    );
  }
...
function compileNode(node, context) {
  ...
  if (node.type === "AND" || node.type === "OR") {
    ...
    const compiledChildren = node.children.map(
      (child) => `(${compileNode(child, context)})`
    );
...
export function buildClickHouseProductIdQuery({
  ...
  const subquery = buildProductIdSubquery({ where, sort });

  return {
    sql: `
      SELECT product_id
      FROM (${subquery.sql}) AS filtered_products
      ORDER BY ${subquery.orderBy}
      LIMIT ${safeLimit}
      OFFSET ${safeOffset}
    `.trim(),
    params,
