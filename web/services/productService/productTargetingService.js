...
async function validateMirrorBatchOwnership({
  ...
  if (requireActive && store.activeMirrorBatchId !== safeMirrorBatchId) {
    throw new Error("Active mirror batch changed before target resolution completed");
  }

  if (store.activeMirrorBatchId && store.activeMirrorBatchId === safeMirrorBatchId) {
    return store;
  }
  ...
  return prisma.$transaction(async (tx) => {
    const pinnedMirrorBatchId =
      typeof requestedMirrorBatchId === "string"
        ...
        : "";
    const mirrorBatchId = pinnedMirrorBatchId || await getActiveMirrorBatchId(safeShop, tx);

    if (!mirrorBatchId) {
      throw new Error("Active mirror batch is required for deterministic targeting");
    }
    ...
    const baseWhere = explicitWhere || getProductPrismaWhere(filterParams, safeShop);
...
export async function getFrozenTargetProductIds({
  ...
  const safeShop = assertShop(shop);
  if (!ownerType || !ownerId) {
    throw new Error("ownerType and ownerId are required to read frozen targets");
  }
  ...
  return {
    ...
    lastOrdinal: rows.length ? rows[rows.length - 1].ordinal : Number(cursorOrdinal) || 0,
