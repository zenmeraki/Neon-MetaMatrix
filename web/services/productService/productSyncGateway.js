...
export async function runProductBulkFetch({ session }) {
  if (typeof graphqlProductsBulkSyncQuery !== "string") {
    throw new Error("graphqlProductsBulkSyncQuery must be a string");
  }

  const queryBody = graphqlProductsBulkSyncQuery.trim();
